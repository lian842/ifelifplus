...
| # | 단계 | 트리거 | 누가 판단 | 사용자 |
| :-: | --- | --- | --- | :-: |
| ① | 상시 메모리 스캔 | 15분 주기 | 🤖 에이전트 | — |
| ② | 관찰 | 상품 페이지 진입 | ⚙️ 규칙 | — |
| ③ | 감지 · 개입 방식 결정 | 결제 버튼 클릭 | ⚙️ 규칙 | — |
| ④ | 질문 설계 | ③이 `challenge`일 때 | 🤖 에이전트 | — |
| ⑤ | 플래닝 모드 (자율 조사) | 사용자 답변 | 🤖 에이전트 | 👤 **1회** |
| ⑥ | 판정 | ⑤ 종료 | ⚙️ **규칙 (LLM 아님)** | — |
| ⑦ | 이력 기록 | 구매 성사 | ⚙️ 규칙 | 👤 **1회** |
| ⑧ | 보류 재검토 | 예약 시각 도달 | 🤖 에이전트 | — |

> ⚙️ = 결정론적 코드 · 🤖 = LLM 판단 · 👤 = 사용자 입력
...
flowchart TB
    ...
    D["③ 감지<br/>상품 · 다크패턴 · 체류시간 · 예산<br/>⚙️ 결정론 · LLM 없음"]:::rule
    D --> M{"이례적 신호가<br/>있는가?"}:::rule

    M -->|"없음<br/>· 의약품<br/>· 5천원 이하<br/>· 이례 신호 0개"| A1
    M -->|"있음<br/>예산초과 · 재구매<br/>다크패턴 · 즉시결제"| B1

    subgraph A["🅰️ price_only ─ 심문 없음 · 마찰 금지"]
        direction TB
        A1["🤖 에이전트가 즉시 최저가 조사"]:::agent
        A1 --> A2["💬 팝업<br/>잔액 · 최저가 · 대안"]:::ui
    end

    subgraph B["🅱️ challenge ─ 심문 → 판정 → 마찰"]
        direction TB
        B1["④ 🤖 질문 설계<br/>형식(객관식/주관식)과 선택지 결정"]:::agent
        B1 --> B2["💬 팝업 1<br/>잔액 · 다크패턴 · 질문<br/>👤 답변 + 대안 Yes/No"]:::ui
        B2 --> B3["⑤ 🤖 플래닝 모드<br/>메모리 + web 자율 조사<br/>20~40초"]:::agent
        B3 --> B4["⑥ ⚙️ 규칙 엔진 판정<br/>PASS · WARN · HOLD · STRONG_HOLD"]:::rule
        B4 --> B5["💬 팝업 2<br/>판정 · 점수근거 · 절약액<br/>👤 최종 결정"]:::ui
    end

    A2 --> F{"사용자 선택"}
    B5 --> F
    F -->|"구매"| REC["⑦ 이력 기록"]:::rule
    F -->|"보류 수용"| HOLD["release_at 등록"]:::rule

    classDef rule fill:#e8eaf0,stroke:#4a5568,color:#1a202c
    classDef agent fill:#e3f0ff,stroke:#2b6cb0,color:#1a202c
    classDef ui fill:#fff4d6,stroke:#b7791f,color:#1a202c
```
...
두 개의 루프가 사용자와 무관하게 돈다. 이것이 "에이전트인가"에 대한 답이다.

```mermaid
flowchart LR
    subgraph L1["① 상시 메모리 스캔 · 15분 주기"]
        direction TB
        S1["소득 · 소비 · 캘린더<br/>결제내역 조회"] --> S2["🤖 기억할 가치 판단"]
        S2 --> S3["write_memory"]
    end

    subgraph L2["⑧ 보류 재검토 · 예약 시각 도달"]
        direction TB
        R1["가격 재확인 (web)<br/>그 사이 유사 구매 조회"] --> R2["🤖 자율 결정"]
        R2 --> R3["release_ok<br/>keep_hold<br/>close"]
    end
    ...
    S3 --> MEM
    R3 --> MEM
    MEM -.->|"다음 개입에 사용"| USE["④ 질문 설계<br/>⑤ 플래닝 모드"]
    HIST -.->|"다음 개입에 사용"| USE
    REC2["⑦ 구매 성사"] --> HIST

    classDef store fill:#f0e6ff,stroke:#6b46c1,color:#1a202c
    class MEM,HIST store
```

**학습이 닫히는 지점**: ⑦에서 기록된 구매가 ④의 선택지로 되돌아온다.
데모 중 같은 물건을 3번 사면, 3번째 질문에 **사용자가 1·2번째에 직접 쓴 문장이 인용된다.**
...
| 모드 | 클릭 | 내역 |
| --- | :-: | --- |
| 🅰️ `price_only` | **1회** | 확인 버튼만 |
| 🅱️ `challenge` | **2회** | ① 질문 답변 ② 최종 결정 |
| 🔄 자율 루프 | **0회** | 에이전트가 스스로 깨어남 |

도구 선택 · 조사 반복 · 종료 시점 · 대안 방식 · 메모리 기록은 **전부 에이전트가 결정한다.**
